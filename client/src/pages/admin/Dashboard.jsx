...
const Dashboard = () => {
    ...
    const [selectedIds, setSelectedIds] = useState([]); // Array of teamIds
    ...
    return (
        <div className="p-8 min-h-screen bg-slate-50">
            <div className="flex justify-between items-center mb-8 bg-white p-6 border-l-4 border-primary rounded-xl shadow-md">
                ...
                <div className="space-x-4 flex items-center flex-wrap gap-2">
                    <div className="flex gap-2 flex-wrap">
                        ...
                        <button onClick={handleExportClick} className="btn-primary text-sm flex items-center gap-1 px-4 py-2 shadow-lg hover:shadow-xl transition-all">
                            {selectedIds.length > 0 ? `Export Selected (${selectedIds.length})` : 'Export Options'}
                        </button>
                    </div>
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-200">
                        {filteredTeams.map(team => (
                            <tr key={team._id} className={`hover:bg-slate-50 transition-colors ${selectedIds.includes(team.teamId) ? 'bg-primary/5' : 'bg-white'}`}>
                                <td className="p-4">
                                    <input
                                        type="checkbox"
                                        className="w-5 h-5 rounded border-slate-300 text-primary focus:ring-primary cursor-pointer accent-primary"
                                        checked={selectedIds.includes(team.teamId)}
                                        onChange={() => handleSelectOne(team.teamId)}
                                    />
                                </td>
            ...
            {isExportModalOpen && (
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
                    <div className="bg-white rounded-xl max-w-2xl w-full p-6 relative shadow-2xl animate-fadeIn">
